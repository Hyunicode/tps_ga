...
Berlin52. 作者：wying 单位：华南理工大学软件学院
*/
...
//
#define MAX_TOWNS 100 // 旅行商问题中城市的最大个数
...
int TSPCrossover(const GAGenome&, const GAGenome&, GAGenome*,
	GAGenome*); // 针对TSP问题的染色体交叉算子

void writeTSPPath(ostream& os,
	GAGenome& g); // 将指定染色体的旅行路线输出到指定文件

int main()
{
	cout << "The GAlib program for the Travelling Salesman Problem (TSP) "
		"Berlin52.\n"
		<< endl;
	...
	ga.nReplacement(2);
	ga.nGenerations(
		500000); // 选用稳态遗传算法进行TSP问题求解，指定其染色体编码方式、每一代要替换的个体数=2、总的运行代数500000，那么搜索的总个体数=2*500000=1000000
	// GASimpleGA ga(genome); ga.elitist(gaTrue);
	// ga.nGenerations(5000);//选用简单遗传算法进行TSP问题求解，采用精英保留策略，指定其染色体编码方式、总的运行代数10000，那么搜索的总个体数=200（种群大小）*5000=1000000
...
void TSPInitializer(GAGenome& g)
{
	...
	town = GARandomInt(1,
		genomelength); // GARandomInt(1,genomelength)生成1到genomelength之间的一个均匀随机整数
	visit[town - 1] = true;
...
int TSPCrossover(const GAGenome& g1, const GAGenome& g2, GAGenome* c1, GAGenome* c2)
{
	...
	for (int i = 0; i < genomelength; i++)
	{
		if (i >= start && i <= end)
			continue;
		while (visit1[parent1.gene(index1) - 1] == 1)
			index1++;
		while (visit2[parent2.gene(index2) - 1] == 1)
			index2++;
	...
	return nc;
	// 使用两点交叉策略
	// 1. 从父代染色体中随机选择两个交叉点, 交叉点之间的基因序列进行交换, 并将交换过的基因标记为已访问
	// 2. 将生成子代的染色体中未访问的基因按照父代染色体的顺序插入到子代染色体中未交换的位置
}
...
// 遗传算法的总体流程:
// 1. 用52个整数(城市)的序列表示染色体, 定义适应度函数为路线欧式距离之和
// 2. 开始迭代进化, 每一代选取2个染色体,用交叉算子和变异算子对染色体进行操作, 并将生成的染色体加入到种群中
// 3. 重复2, 并采样记录每次的最优解, 直到达到迭代次数(500000)
